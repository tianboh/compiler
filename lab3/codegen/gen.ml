      ...
      | h :: t -> 
        ...
        | AS.Mov mov -> 
          ...
          | AS.Temp _, AS.Temp src -> 
            ...
            | Some s -> helper t (res @ [AS.Mov{dest=mov.dest;src=AS.Imm s}]) const_map)
          | _ -> helper t (res @ [h]) const_map)
        | _ -> helper t (res @ [h]) const_map
      ...
      | h :: t -> 
        ...
        | AS.Mov mov -> (match mov.dest, mov.src with
          | AS.Temp dest, AS.Temp src -> (
            if Temp.is_reg dest
              then 
                let src = match Temp.Map.find map src with | None -> src | Some s -> s in
                helper t (ret@[AS.Mov{dest=AS.Temp dest; src=AS.Temp src}]) map
              else 
                let root = match Temp.Map.find map src with | None -> src | Some s -> s in
