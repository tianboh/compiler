...
let build_def_lo adj def s_lo =
  ...
  Temp.Set.fold_right s_lo ~init:adj ~f:(fun v adj ->
      let s_v =
        match Temp.Map.find adj v with None -> Temp.Set.empty | Some s -> s
      in
...
let gen_reg_table prog adj =
  ...
  in
  let reg_table =
    List.fold pre_color_l ~init:reg_table ~f:(fun acc x ->
        Temp.Map.remove acc x)
  in
...
let rec greedy seq adj tmp_to_reg =
  ...
  | h :: t ->
      ...
      if Temp.value h < 0 then greedy t adj tmp_to_reg
      else
  ...
  | h :: t -> (
      ...
      | Some tmp ->
          if Temp.value tmp > 0 then
            ...
            let tk =
              match reg with None -> None | Some reg' -> Some (tmp, reg')
            in
