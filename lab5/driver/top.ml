...
let cmd_line_term : cmd_line_args Cmdliner.Term.t =
  ...
  and emit =
    ...
    opt Emit.conv ~default:Emit.Quad
      (Arg.info [ "e"; "emit" ] ~doc ~docv:"TARGET")
  and opt_level =
    let doc = "[0|1|2] The optimization level $(docv)." in
    opt Opt_level.conv ~default:Opt_level.Opt_none
      (Arg.info [ "O"; "opt" ] ~doc ~docv:"OPT")
  and df_type =
    let doc =
      "[forward-may|forward-must|backward-may|backward-must|no-analysis] The \
       type of dataflow analysis"
    in
    opt Df_analysis.conv ~default:Df_analysis.No_analysis
  ...
  and header_file =
    let info =
      Arg.info [ "l"; "link" ] ~docv:"NUM"
        ~doc:"The header file $(docv) to compile."
    in
...
let compile (cmd : cmd_line_args) : unit =
  ...
  | Quad ->
      ...
      say_if cmd.verbose (fun () ->
          sprintf "Writing abstract assem to %s..." file);
...
let main () =
  ...
  let cmd =
    Cmd.v (Cmd.info "c0c" ~doc:"Compile a c0c source file.") cmd_line_term_unit
  in
