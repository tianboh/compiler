...
type field = { dtype : TST.dtype; offset : Int64.t; size : Size.t }
type c0_struct = { fields : field Map.t; size : Size.t }
type var = { temp : Temp.t; dtype : TST.dtype }
...
type ldest = Mem of Mem.t | Temp of Temp.t
...
let rec align (dtype : TST.dtype) (env : env) : Size.t =
  ...
  | `Array _ -> `QWORD
  | `Struct sname ->
      if Map.mem !alignment sname then Map.find_exn !alignment sname
      else
...
let is_large = function `Struct _ -> true | _ -> false

let get_struct_exn (dtype : TST.dtype) : Symbol.t =
  match dtype with `Struct s -> s | _ -> failwith "expect dtype as struct"

let check_base_size (base : Sexp.t) =
  if Size.compare' base.size `QWORD <> 0 then failwith "base should be qword"

let check_index_size (index : Sexp.t) =
  if Size.compare' index.size `DWORD <> 0 then failwith "index should be dword"
...
let check_bound (base : Sexp.t) (index : Sexp.t) : Tree.stm list * Tree.stm list
  ...
  let mem_header : Mem.t =
    Addr.of_bisd base None None (Some (-8L)) |> Mem.wrap `QWORD
  in
  let arr_size_t = Temp.create index.size |> Exp.of_t |> Sexp.wrap index.size in
  let arr_size_s = Sexp.to_t arr_size_t in
...
let trans_exp_bin (binop : TST.binexp) (env : env) trans_func :
  ...
  | `Impure op ->
      ...
      ( lhs_stm @ rhs_stm @ [ Tree.Effect { dest; lhs; op; rhs } ],
        Inst.t_to_Sexp dest )
...
let trans_fcall (fcall : TST.fcall) (env : env) trans_func =
  ...
  let func_name =
    { func_name with name = Symbol.pp_scope scope ^ func_name.name }
  in
  ...
  | _ ->
      ...
      let call_stms = args_stms @ [ call ] in
      (call_stms, Inst.t_to_Sexp dest)

let trans_alloc (alloc : TST.alloc) env =
  let size = `DWORD in
  let size_32 =
    sizeof_dtype alloc.dtype env
    |> Size.type_size_byte |> Exp.of_int |> Sexp.wrap size
  in
  ...
  ([ Tree.Fcall { dest = Some ptr; func_name; args } ], Inst.t_to_Sexp ptr)

(* alloc size should >= 0 *)
let check_alloc_arr (size : Sexp.t) : Tree.stm list =
  let lhs, op, rhs =
    (size, Tree.Greater_eq, Sexp.wrap `DWORD (Exp.of_int 0L))
  in
...
let trans_alloc_arr (alloc_arr : TST.alloc_arr) env trans_func need_check =
  ...
  let size_tot =
    Addr.of_bisd
      (Sexp.wrap size_4 (Exp.of_int 8L))
      (Some nitems) (Some esize) None
  in
  let args =
    [
      1L |> Exp.of_int |> Sexp.wrap size_4;
      size_tot |> Exp.of_bisd |> Sexp.wrap size_4;
    ]
  in
  let size_8 = `QWORD in
  let ptr_c0 = Temp.create size_8 |> Exp.of_t |> Sexp.wrap size_8 in
  let alloc = Tree.Fcall { dest = Some (Sexp.to_t ptr_c0); func_name; args } in
  let header = Mem.wrap size_8 (Addr.of_bisd ptr_c0 None None None) in
  let store_size =
    Tree.Store { dest = header; src = { nitems with size = size_8 } }
  in
  let ptr_c = Temp.create size_8 in
  let c_addr =
    Addr.of_bisd ptr_c0 None None (Some 8L) |> Exp.of_bisd |> Sexp.wrap size_8
  in
  let move = Tree.Move { dest = ptr_c; src = c_addr } in
  (stms @ check @ [ alloc; store_size; move ], Inst.t_to_Sexp ptr_c)

let trans_mem (base : Sexp.t) (index : Sexp.t option) (scale : Int64.t option)
    (disp : Int64.t option) (size : Size.primitive) : Tree.stm list * Sexp.t =
  let src = Addr.of_bisd base index scale disp |> Mem.wrap size in
  let dest = Temp.create size in
  let load = Tree.Load { src; dest } in
  ([ load ], Inst.t_to_Sexp dest)

(* Return small value, or memory calculation formula *)
let[@warning "-8"] rec trans_exp (need_check : bool) (exp_tst : TST.texp)
    (env : env) : Tree.stm list * Sexp.t =
  let size = sizeof_dtype' exp_tst.dtype in
  match exp_tst.data with
  | `Dot dot ->
      trans_dot need_check
        ({ data = dot; dtype = exp_tst.dtype } : TST.dot TST.typed)
        env
  | `Nth nth ->
      trans_nth need_check
        ({ data = nth; dtype = exp_tst.dtype } : TST.nth TST.typed)
        env
...
and trans_nth need_check (nth : TST.nth TST.typed) env : Tree.stm list * Sexp.t
  ...
  let index = Temp.create `QWORD |> Exp.of_t |> Sexp.wrap `QWORD in
  let index_stm =
    index_stm @ [ Tree.Cast { dest = Sexp.to_t index; src = index_exp } ]
  in
  let scale = sizeof_dtype nth.dtype env |> Size.type_size_byte in
  let base_check, bound_check =
    if need_check then check_bound base index else ([], [])
  in
  if is_large nth.dtype then
    ( base_stm @ index_stm @ base_check @ bound_check,
      Addr.of_bisd base (Some index) (Some scale) None
      |> Exp.of_bisd |> Sexp.wrap `QWORD )
  else
...
and trans_dot need_check (dot : TST.dot TST.typed) env : Tree.stm list * Sexp.t
  ...
  let base_stm, base =
    ...
    | `Nth nth ->
        trans_nth need_check
          ({ data = nth; dtype = s'.dtype } : TST.nth TST.typed)
          env
    | _ -> failwith "hmmm, should not happen"
  in
  check_base_size base;
  let check = if need_check then check_null base else [] in
  if is_large dot.dtype then
    if Int64.equal field.offset 0L then (base_stm, base)
...
and trans_postop need_check (pop : TST.postexp) (env : env) =
  ...
  let assign =
    match pop.op with
    | `Plus_plus ->
        TST.Assign { name = pop.operand; value = one; op = `Plus_asn }
    | `Minus_minus ->
        TST.Assign { name = pop.operand; value = one; op = `Minus_asn }
  in
...
and trans_lvalue (lvalue : TST.texp) (env : env) (need_check : bool) :
  ...
  | `Dot dot ->
      let base_stms, base =
        ...
        | `Deref deref -> trans_exp need_check deref.ptr env
        | `Dot dot' ->
            trans_dot need_check { data = dot'; dtype = s'.dtype } env
        | `Nth nth ->
            trans_nth need_check
              ({ data = nth; dtype = s'.dtype } : TST.nth TST.typed)
              env
      ...
      let addr_check =
        if need_check then check_null (addr |> Exp.of_bisd |> Sexp.wrap `QWORD)
        else []
      in
  ...
  | `Deref deref ->
      ...
      ( base_stms,
        check,
        Mem (Addr.of_bisd base None None None |> Mem.wrap size),
        `Deref )
  | `Nth nth ->
...
and[@warning "-8"] trans_assign_rev acc (TST.Assign asn_tst) need_check
    (env : env) : Tree.stm list =
  let dest_stms, dest_check, lhs, ltype =
    trans_lvalue asn_tst.name env need_check
  in
  let v_stms, rhs = trans_exp need_check asn_tst.value env in
  match lhs with
  | Temp t ->
      let src = rhs in
      (Tree.Move { dest = t; src } :: List.rev v_stms) @ acc
  | Mem dest -> (
      let src, cast =
        if Size.compare' dest.size rhs.size <> 0 then
          let dest' = Temp.create dest.size in
          ( Exp.of_t dest' |> Sexp.wrap dest.size,
            [ Tree.Cast { dest = dest'; src = rhs } ] )
        else (rhs, [])
      in
      match ltype with
      | `Nth ->
          ((Tree.Store { dest; src } :: cast) @ List.rev v_stms)
          @ List.rev dest_check @ List.rev dest_stms @ acc
      | `Deref | `Dot ->
          (((Tree.Store { dest; src } :: cast) @ List.rev dest_check)
          @ List.rev v_stms)
          @ List.rev dest_stms @ acc
      | `Var -> failwith "should not happen")

and[@warning "-8"] trans_assignop_rev acc (TST.Assign asn_tst) need_check
    (env : env) p_ip op : Tree.stm list =
  let size = sizeof_dtype' asn_tst.value.dtype in
  let dest_stms, dest_check, lhs, _ =
    trans_lvalue asn_tst.name env need_check
  in
  let v_stms, rhs = trans_exp need_check asn_tst.value env in
  match lhs with
  | Temp t -> (
      ...
      | `Pure ->
          let src =
            Sexp.wrap size (Exp.of_binop op (Inst.t_to_Sexp dest) rhs)
          in
          (Tree.Move { dest; src } :: List.rev v_stms) @ acc
      | `Impure ->
          Tree.Effect { dest; lhs = Inst.t_to_Sexp dest; rhs; op }
          :: List.rev v_stms
          @ acc)
  | Mem mem ->
      let src, src_stm =
        ...
        | `Pure ->
            ...
            let lhs_temp_stms, lhs_temp =
              ([ Tree.Load { src = mem; dest } ], Inst.t_to_Sexp dest)
            in
            ( Sexp.wrap size (Exp.of_binop op lhs_temp rhs),
              dest_check @ lhs_temp_stms )
        | `Impure ->
            ...
            ( Inst.t_to_Sexp t,
              ...
              @ [
                  ...
                  Tree.Effect { dest = t; lhs = Inst.t_to_Sexp t; rhs; op };
                ] )
      in
      ((Tree.Store { dest = mem; src } :: List.rev src_stm) @ List.rev v_stms)
      @ List.rev dest_stms @ acc

and[@warning "-8"] trans_asnop_rev acc (TST.Assign asn_tst) need_check
    (env : env) : Tree.stm list =
...
let rec trans_stm_rev (tst : TST.stm) (acc : Tree.stm list) (env : env)
    (need_check : bool) : Tree.stm list * env =
  match tst with
  | TST.Assign asn_tst ->
      (trans_asnop_rev acc (TST.Assign asn_tst) need_check env, env)
  | TST.If if_TST ->
      (*
...
let padding (dtype : TST.dtype) (offset : Int64.t) (env : env) : Int64.t =
  ...
  if tsize_64 > 0L then
    match offset % tsize_64 with 0L -> 0L | m -> tsize_64 - m
  else 0L
...
let trans_struct (s : TC.c0_struct) (env : env) : c0_struct =
  ...
  let fields, size =
    List.fold s.fields ~init:(Map.empty, 0L) ~f:(fun acc field ->
        ...
        let offset_next =
          if offset_next > Int64.of_int32_exn Int32.max_value then 0L
          else offset_next
        in
...
let trans_fdefn func_name (pars : TST.param list) blk (env : env)
    (need_check : bool) : Tree.fdefn =
  let vars =
  ...
  let temps : Temp.t list =
    List.map pars ~f:(fun par ->
        let var = Map.find_exn env.vars par.data in
        let var_s = var.temp in
        var_s)
  in
  { func_name; temps; body }

let rec trans_prog (program : TST.program) (acc : Tree.program) (env : env)
    (need_check : bool) : Tree.program =
  match program with
  | [] -> List.rev acc
  | fdefn :: t ->
      ...
      let func_name =
        {
          fdefn.func_name with
          name = Symbol.pp_scope func.scope ^ fdefn.func_name.name;
        }
      in
      let fdefn_tree =
        trans_fdefn func_name fdefn.pars fdefn.blk env need_check
      in
      trans_prog t (fdefn_tree :: acc) env need_check

let translate (program : TST.program) (env : TC.env) (unsafe : bool) :
    Tree.program =
