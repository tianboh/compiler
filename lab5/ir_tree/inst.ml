...
and Exp : sig
  type t =
    ...
    | Const of Int64.t
    | Temp of Temp.t
    | Binop of { lhs : Sexp.t; op : binop; rhs : Sexp.t }
    | BISD of Addr.t
  ...
  let pp exp =
    ...
    | Temp t -> Temp.name t
    | Binop binop ->
        Printf.sprintf "(%s %s %s)" (Sexp.pp binop.lhs) (pp_binop binop.op)
          (Sexp.pp binop.rhs)
    | BISD bisd -> Addr.pp bisd

  let to_t exp = match exp with Temp t -> t | _ -> failwith "expect temp"
...
let is_return = function Return _ -> true | _ -> false
...
let get_label (stm : stm) : Label.t =
  match stm with Label l -> l | _ -> failwith "expect instr to be label"

(* Given jump/conditional jump, return target label list. *)
let get_targets (instr : stm) : Label.t list =
  match instr with
  | Jump jp -> [ jp ]
...
let replace_ctarget (instr : stm) (old_target : Label.t) (new_target : Label.t)
    : stm =
  match instr with
  | CJump cjp ->
...
module Print = struct
  ...
  let rec pp_stm = function
    ...
    | CJump cj ->
        sprintf "cjump(%s %s %s) target_true:%s, target_false %s " (pp cj.lhs)
          (Exp.pp_binop cj.op) (pp cj.rhs)
