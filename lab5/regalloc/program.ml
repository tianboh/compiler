...
let rec gen_forward (inst_list : Abs_asm.instr list)
    (inst_info : (int, line * Abs_asm.instr) Base.Hashtbl.t) (line_num : int)
    live_out_map =
  ...
  | h :: t -> (
      ...
      | Abs_asm.Binop binop ->
          let inst_info =
            helper binop.line live_out false inst_info h line_num
          in
      ...
      | Abs_asm.Label label ->
          let inst_info =
            helper label.line live_out false inst_info h line_num
          in
          gen_forward t inst_info (line_num + 1) live_out_map
      | Abs_asm.Fcall fcall ->
          let inst_info =
            helper fcall.line live_out false inst_info h line_num
          in
          gen_forward t inst_info (line_num + 1) live_out_map
      | Abs_asm.Pop pop ->
      ...
      | Abs_asm.Push push ->
          let inst_info =
            helper push.line live_out false inst_info h line_num
          in
          gen_forward t inst_info (line_num + 1) live_out_map
      | Abs_asm.Load load ->
          let inst_info =
            helper load.line live_out false inst_info h line_num
          in
          gen_forward t inst_info (line_num + 1) live_out_map
      | Abs_asm.Store store ->
          let inst_info =
            helper store.line live_out false inst_info h line_num
          in
          gen_forward t inst_info (line_num + 1) live_out_map
...
let gen_regalloc_info (inst_list : Abs_asm.instr list) =
  ...
  let liveness = Liveness_old.gen_liveness inst_list in
