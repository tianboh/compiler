...
let rec gen_succ (inst_list : Abs_asm.instr list) (line_no : int) map =
  ...
  | h :: t -> (
      ...
      | Jump _ | CJump _ | Ret _ | Mov _ | Cast _ | Binop _ | Fcall _ | Push _
      | Pop _ | Store _ | Load _ ->
          gen_succ t (line_no + 1) map
...
let[@warning "-8"] _gen_df_info_cjump line_number (Abs_asm.CJump cjump)
    label_map : Dfana_info.line =
...
let rec _gen_df_info_rev (inst_list : Abs_asm.instr list) line_number label_map
    res =
  match inst_list with
  | [] -> res
  | h :: t -> (
      let line =
        ...
        | CJump cjp ->
            Some (_gen_df_info_cjump line_number (CJump cjp) label_map)
        | Ret ret ->
            let gen, kill =
              ( Abs_asm.to_int_list ret.line.uses,
                Abs_asm.to_int_list ret.line.defines )
            in
            Some { gen; kill; succ = []; is_label = false; line_number }
        | Label _ ->
            let succ = [ line_number + 1 ] in
      ...
      | Some line_s ->
          _gen_df_info_rev t (line_number + 1) label_map (line_s :: res))
