...
let[@warning "-8"] _gen_df_info_helper (line_number : int) (line : Line.t) =
  ...
  let gen = Int.Set.of_list (Inst.to_int_list uses) in
  let kill = Int.Set.of_list (Inst.to_int_list defs) in
...
let rec _gen_df_info_rev (instrs : (Inst.instr * Line.t) list) line_number label_map res =
  ...
  | h :: t ->
    ...
    let line' : Dfana_info.line option =
      ...
      then (
        let gen, kill = Inst.to_int_list line.uses, Inst.to_int_list line.defines in
