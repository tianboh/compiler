...
module Op = struct
  type t =
    ...
    | Stack of { index : Int64.t; reg : Register.t }
  ...
  let pp = function
    ...
    | Stack s ->
        sprintf "(%s, %Ld, 8)" (Register.reg_to_str' s.reg `QWORD) s.index
...
type instr =
  | Add of { src : Sop.t; dest : Sop.t; size : Size.primitive }
  | Sub of { src : Sop.t; dest : Sop.t; size : Size.primitive }
  | Mul of { src : Sop.t; dest : Sop.t; size : Size.primitive }
    (* Destination is form of EDX:EAX by default. Only one Sop.t required. *)
  | Div of { src : Sop.t; size : Size.primitive }
  (* temp := EDX:EAX / SRC;
            IF temp > FFFFFFFFH
            ...
            ELSE
                ...
                EDX := EDX:EAX MOD SRC;
            FI; *)
  | Mod of { src : Sop.t; size : Size.primitive }
    (* Similar as above, but use edx after div.*)
  | Cast of { dest : Sop.t; src : Sop.t }
  | Mov of {
      dest : Sop.t;
      src : Sop.t;
      size : Size.primitive;
  ...
  | Cvt of { size : Size.primitive }
    (*could be cdq, cqo, etc based on size it wants to extend. EDX:EAX := sign-extend of EAX *)
  | Ret
  | Pop of { var : Sop.t }
  | Push of { var : Sop.t }
  | Cmp of { lhs : Sop.t; rhs : Sop.t; size : Size.primitive }
  ...
  | SETNE of { dest : Sop.t; size : Size.primitive }
  | SETL of { dest : Sop.t; size : Size.primitive }
  | SETLE of { dest : Sop.t; size : Size.primitive }
  | SETG of { dest : Sop.t; size : Size.primitive }
  | SETGE of { dest : Sop.t; size : Size.primitive }
  | AND of {
      (* bitwise and *)
      src : Sop.t;
      dest : Sop.t;
  ...
  | XOR of { src : Sop.t; dest : Sop.t; size : Size.primitive }
  | SAL of {
...
type fdefn = { func_name : Symbol.t; body : instr list }
...
let format = function
  ...
  | Add add ->
      sprintf "add%s %s, %s" (pp_inst add.size) (pp_Sop add.src)
        (pp_Sop add.dest)
  | Sub sub ->
      sprintf "sub%s %s, %s" (pp_inst sub.size) (pp_Sop sub.src)
        (pp_Sop sub.dest)
  | Mul mul ->
      sprintf "imul%s %s, %s" (pp_inst mul.size) (pp_Sop mul.src)
        (pp_Sop mul.dest)
  ...
  | Cast cast ->
      ...
      if Size.compare' src_size dest_size = 0 then
        failwith "cast oprd size match";
      if Size.compare' src_size dest_size < 0 then
  ...
  | Mov mv -> (
      ...
      let () =
        if Size.compare' mv.src.size mv.dest.size <> 0 then
          printf "move oprd size mismatch mov%s %s, %s\n" (pp_inst mv.dest.size)
            src_str dest_str
        else ()
      in
  ...
  | Cmp cmp ->
      sprintf "cmp%s %s, %s" (pp_inst' cmp.lhs) (pp_Sop cmp.rhs)
        (pp_Sop cmp.lhs)
  | LAHF -> "lahf"
  | SAHF -> "sahf"
  ...
  | AND a ->
      sprintf "and%s %s, %s" (pp_inst a.size) (pp_Sop a.src) (pp_Sop a.dest)
  | OR a ->
      sprintf "or%s %s, %s" (pp_inst a.size) (pp_Sop a.src) (pp_Sop a.dest)
  | XOR xor ->
      sprintf "xor%s %s, %s" (pp_inst' xor.dest) (pp_Sop xor.src)
        (pp_Sop xor.dest)
  | SAR sar ->
      sprintf "sar%s %s, %s" (pp_inst sar.size) (pp_Sop sar.src)
        (pp_Sop sar.dest)
  | SAL sal ->
      sprintf "sal%s %s, %s" (pp_inst sal.size) (pp_Sop sal.src)
        (pp_Sop sal.dest)
