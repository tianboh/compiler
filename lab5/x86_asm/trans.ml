...
let trans_operand (operand : Src.Sop.t)
    (reg_alloc_info : Regalloc.dest IG.Vertex.Map.t) : Dest.Sop.t =
  ...
  | Temp t -> (
      ...
      | Regalloc.Spill s ->
          ...
          -id |> Int64.of_int
          |> Dest.Op.of_stack Reg_logic.RBP
          |> Dest.Sop.wrap size)
...
let safe_cast (dest : Dest.Sop.t) (src : Dest.Sop.t) =
  if Size.compare (dest.size :> Size.t) (src.size :> Size.t) = 0 then
    ...
    | Dest.Op.Stack _, Dest.Op.Stack _ ->
        ...
        let swap = Reg_logic.swap |> Dest.Op.of_reg |> Dest.Sop.wrap size in
        let swap_ext =
          Reg_logic.swap |> Dest.Op.of_reg |> Dest.Sop.wrap dest.size
        in
        [
    ...
    | Dest.Op.Stack _, Dest.Op.Reg r ->
        ...
        [
          Dest.Cast
            { dest = r_ext; src = Dest.Op.of_reg r |> Dest.Sop.wrap src.size };
...
let safe_cmp (lhs : Dest.Sop.t) (rhs : Dest.Sop.t) (size : Size.primitive)
    (swap : Dest.Sop.t) =
  match (lhs.data, rhs.data) with
  | Dest.Op.Stack _, Dest.Op.Stack _ ->
      [
        Dest.Mov { dest = swap; src = lhs; size };
        Dest.Cmp { lhs = swap; rhs; size };
      ]
  | _ -> [ Dest.Cmp { lhs; rhs; size } ]

let gen_x86_relop_bin (op : Src.binop) (dest : Dest.Sop.t) (lhs : Dest.Sop.t)
    (rhs : Dest.Sop.t) (swap : Dest.Sop.t) =
  ...
  [ Dest.XOR { dest = rax; src = rax; size } ]
  @ cmp_inst @ set_inst @ safe_cast dest rax

let gen_x86_inst_bin (op : Src.binop) (dest : Dest.Sop.t) (lhs : Dest.Sop.t)
    (rhs : Dest.Sop.t) (swap : Reg_logic.t) =
...
let rec _codegen_w_reg_rev (res : Dest.instr list) (inst_list : Src.instr list)
    (reg_alloc_info : Regalloc.dest IG.Vertex.Map.t) (reg_swap : Reg_logic.t) =
  ...
  | h :: t -> (
      ...
      | Label l ->
          _codegen_w_reg_rev
            (Dest.Label l.label :: res)
            t reg_alloc_info reg_swap
      | Jump jp ->
          _codegen_w_reg_rev
            (Dest.Jump jp.target :: res)
            t reg_alloc_info reg_swap
      | CJump cjp ->
      ...
      | Load load ->
          ...
          let insts_rev =
            ...
            | Dest.Op.Stack _ ->
                let swap =
                  Reg_logic.swap |> Dest.Op.of_reg |> Dest.Sop.wrap size
                in
                [
      ...
      | Directive _ | Comment _ ->
          _codegen_w_reg_rev res t reg_alloc_info reg_swap)

let abort_handler = [ Dest.Label abort_label; Dest.Abort ]

(* Transform operand with stack to address. *)
let rec transform (insts_rev : Dest.instr list) (acc : Dest.instr list) :
    Dest.instr list =
...
let gen (fdefn : Src.fdefn)
  ...
  let mem_cnt_oprd =
    mem_cnt |> Int64.of_int |> Dest.Op.of_imm |> Dest.Sop.wrap size
  in
