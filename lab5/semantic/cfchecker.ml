...
type env = { var_def : Set.t; var_decl : Set.t }
...
let rec cf_ret (ast : AST.program) : unit =
  ...
  | h :: t ->
      ...
      | Fdefn fdefn -> (
          ...
          | `Void -> ()
          | _ ->
              if not (_cf_ret_stm fdefn.blk) then
                error ~msg:"Some branches not return" None)
...
let rec use (exp : AST.mexp) (var : Symbol.t) : bool =
  ...
  | Fcall fcall ->
      List.fold fcall.args ~init:false ~f:(fun acc arg -> acc || use arg var)
...
let rec live (stm : AST.mstm) (var : Symbol.t) : bool =
  ...
  | If if_ast ->
      use if_ast.cond var || live if_ast.true_stm var
      || live if_ast.false_stm var
  | While while_ast -> use while_ast.cond var || live while_ast.body var
  | Return ret_ast -> (
      match ret_ast with None -> false | Some s -> use s var)
  | Nop -> false
  | Seq seq_ast ->
      live seq_ast.head var
      || (live seq_ast.tail var && not (define seq_ast.head var))
  | Declare decl_ast ->
      if phys_equal var decl_ast.name then false
      else
        let c1 = live decl_ast.tail var in
        let c2 =
          match decl_ast.value with None -> false | Some v -> use v var
        in
        c1 || c2
...
let rec cf_exp (exp : AST.mexp) (env : env) : unit =
  ...
  match Mark.data exp with
  | Var var ->
      if not (Set.mem env.var_def var) then
        error ~msg:(sprintf "var %s is used before decl" (Symbol.name var)) loc
  | Binop binop ->
 ...
 *)
let rec cf_lvalue (lvalue : AST.mlvalue) (env : env) (asnop : AST.asnop)
    (need_defn : bool) : unit =
  ...
  | Ident var -> (
      match asnop with
      | `Asn ->
          if not (Set.mem env.var_decl var) then
            error
              ~msg:(sprintf "var %s is defined before decl" (Symbol.name var))
              loc;
          if need_defn && not (Set.mem env.var_def var) then
            error
              ~msg:(sprintf "var %s should be defined" (Symbol.name var))
              loc
      | _ ->
          if not (Set.mem env.var_def var) then
            error
              ~msg:(sprintf "var %s is used before define" (Symbol.name var))
              loc)
...
let rec cf_stm (ast : AST.mstm) (env : env) : env =
  ...
  | Declare decl_ast ->
      ...
      let env' =
        ...
        | None ->
            if live decl_ast.tail var then
              error
                ~msg:(sprintf "var %s is live in decl" (Symbol.name var))
                loc;
            { env with var_decl = Set.add env.var_decl var }
        | Some v ->
            cf_exp v env;
            {
              var_decl = Set.add env.var_decl var;
              var_def = Set.add env.var_def var;
            }
      in
       ...
       * env_inside.var_def may contain some variable declared in inner scope *)
      {
        ...
        var_def = Set.diff env_inside.var_def (Set.singleton var);
      }
  | Sexp sexp ->
      cf_exp sexp env;
      env
...
let init_func_pars (pars : param list) (env : env) : env =
  List.fold_left pars ~init:env ~f:(fun env_acc par ->
      let var = par.i in
      {
        var_def = Set.add env_acc.var_def var;
        var_decl = Set.add env_acc.var_decl var;
      })
